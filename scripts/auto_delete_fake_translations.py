...
def auto_delete_fake_translations():
    """自动删除伪翻译的文章"""
    ...
    try:
        ...
        cursor.execute("""
            ...
            ORDER BY id
            LIMIT 5
        """.format(','.join(map(str, fake_translations[:5]))))
        
        sample_articles = cursor.fetchall()
        
        for article in sample_articles:
            article_id, original_title, translated_title = article
            print(f"ID: {article_id}, 伪翻译: {translated_title[:50]}...")
        
        # 自动删除伪翻译的文章
        print(f"\n=== 开始删除操作 ===")
        print(f"正在删除 {len(fake_translations)} 篇伪翻译的文章...")
        ...
        for article in remaining_samples:
            article_id, translated_title = article
            print(f"ID: {article_id}, 中文标题: {translated_title[:50]}...")
